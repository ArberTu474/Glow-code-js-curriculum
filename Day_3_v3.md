...
- Explain to the girls the concept of `DRY` (Don't repeat Yourself). In programming it is very important to not repeat yourself. We want the machine to do the work for us.
...
- Write a correct condition to make sure that the loop runs exactly 10 times.
...
- The girls should be able to identify the the `index`, the `condition` and the `incrementor`, but they are placed in different places.
...
- Display all the numbers form `5` to `10`, but now by using a `while` loop. By the end of this exercise the girls should be able to "convert" a `while` loop to `for` loop.
...
- Give hints to the girls that they need to use a loop for this. Decide with them if a `for` loop or a `while` loop makes more sense.
...
Modify [Exercise 5](#exercise-5) to take another parameter `name` and include that name to the greeting.
